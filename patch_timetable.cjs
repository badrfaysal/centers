...
let content = fs.readFileSync('resources/views/schedules/specialist_timetable.blade.php', 'utf8');
...
content = content.replace("@push('scripts')", modalHtml + "\\n@push('scripts')");

const alpineData = \        daySummaryModalOpen: false,
        daySessions: [],
        dayTotalHours: '0',
        dayTotalGaps: '0',
        dayIsAllCancelled: false,\;

content = content.replace("detailModalOpen: false,", alpineData + "\\n        detailModalOpen: false,");

const newHandleDayClick = \        handleDayClick(day) {
...
content = content.replace(/handleDayClick\(day\)\s*\{[\s\S]*?\},(?=\n)/, newHandleDayClick + '\\n');

fs.writeFileSync('resources/views/schedules/specialist_timetable.blade.php', content, 'utf8');
console.log('Done');