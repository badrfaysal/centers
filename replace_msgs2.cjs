...
والد الطف؄: {{ $child ? $child->name : 'غير مدد' }})</span>
                ...
                <div x-show="expandedMsg" x-collapse>
                    <div class="p-4 sm:p-5 bg-slate-50/50 border-t border-slate-100 space-y-4">
                        ...
                        <div class="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-xs space-y-4">
                            <div class="flex items-center justify-between gap-3 pb-3 border-b border-slate-50 text-[10px]">
                                <span class="font-mono text-slate-400">
                                    موجه إلى: <strong class="text-slate-700">{{ $pmsg->recipient_type === 'specialist' ? 'الأطصائي المعالج' : 'إدارة المعوض' }}</strong>
                                </span>
                            ...
                            <div class="p-4 rounded-2xl bg-emerald-50/80 border border-emerald-100 space-y-2 pr-6 text-xs">
                            ...
                            <!-- ف؈رم الرد -->
                            <form action="{{ route('doctor.message.reply') }}" method="POST" class="pt-2 space-y-2 pr-6">
                                @csrf
                                <input type="hidden" name="message_id" value="{{ $pmsg->id }}">
                                <input type="hidden" name="doctor_name" value="{{ $pmsg->child ? ($pmsg->child->main_specialist ?? 'ء. معالج نفسن') : 'ء. معالج نفسل' }}">

                                <div class="flex flex-col sm:flex-row gap-2">
                                    <textarea name="doctor_reply" required rows="2" placeholder="{{ $pmsg->doctor_reply ? 'تعديل الرد أو إضافة تفاصيل أخرى للرد السابق...' : 'اكتب ردك وتوجيهك الطبي لولي الأمد هنا...' }}" class="flex-1 p-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:bg-white text-xs font-medium leading-relaxed"></textarea>
                                    <button type="submit" class="px-5 py-2.5 rounded-2xl text-white font-bold text-xs shadow-md transition hover:opacity-90 flex items-center justify-center gap-1.5 shrink-0 self-end" style="background-color: var(--brand-primary, #0d9488);">
                                        ...
                                        <span>{{ $pmsg->doctor_reply ? 'تعديل الرد' : 'إدسال الرد للأهل' }}</span>
                                    </button>
                                </div>
                            </form>
                        </div>
            ...
            <div class="py-12 text-center text-slate-400 text-xs bg-white rounded-3xl border border-slate-100 p-8">
                ...
                <p class="font-black">لا توجدرسائل أتط هد أتطياڌ المن</p>
            </div>
            @endforelse
        </div>`;

        const tabIdx = c.indexOf('<span>التأغدر المعجيو ناو���������(������������Ё�х��%��􁌹�����=����؁�������������Ј�����������=������������ѕ�е�́ѕ�еͱ�є��������(������������Ё���%��􁌹�����=��������ɕ�͔����х��%�ऀ�����(������������х��%�����Ĥ��(��������������􁌹�Չ��ɥ�������х��%�ऀ��ɕ��������Ѐ�����Չ��ɥ������%���(�������������̹�ɥѕ���M幌��ɕͽ�ɍ�̽٥��̽���ѽȽ����้����������������ј���(��������􁕱͔��(���������������ͽ��������9�Ё��չ����(��������
