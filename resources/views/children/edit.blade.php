...
<div class="space-y-8" x-data="{ 
    gender: '{{ $child->gender }}',
    diagnosisCategory: '{{ $child->diagnosis_category }}',
    packageType: '{{ $child->package_type }}',
    photoPreview: '{{ $child->avatar_url }}',
    diagnoses: {{ json_encode(!empty($child->diagnoses_list) ? $child->diagnoses_list : ['']) }},
    addDiagnosis() {
        this.diagnoses.push('');
    },
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-2xl font-black text-slate-800">تعديل ملف الطفل: {{ $child->name }}</h2>
                <p class="text-xs text-slate-500 font-semibold mt-0.5">تحديث البيانات الشخصية، الطبية، التشخيصات المستقلة، والمتابعات</p>
            </div>
        </div>
    ...
    <form action="{{ route('children.update', $child) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        ...
        <div class="bg-white rounded-3xl p-6 md:p-8 border border-slate-100 shadow-sm space-y-8">
            ...
            <div class="flex flex-col md:flex-row items-center justify-between gap-6 p-5 rounded-3xl bg-slate-50 border border-slate-100">
                ...
                <!-- رفع وتحديث صورة الطفل -->
                <div class="flex items-center gap-4">
                    <div class="relative group">
                        <img :src="photoPreview" alt="child photo" class="w-18 h-18 rounded-2xl object-cover ring-4 ring-white shadow-md">
                        <label class="absolute -bottom-2 -left-2 w-7 h-7 rounded-xl bg-white shadow-md border border-slate-200 flex items-center justify-center cursor-pointer hover:bg-slate-50 transition" title="تغيير الصورة">
                            <i class="fa-solid fa-camera text-xs" style="color: #0d9488;"></i>
                            <input type="file" name="photo" accept="image/*" @change="previewImage" class="hidden">
                        </label>
                    </div>

                    <div>
                        <p class="font-extrabold text-sm text-slate-800">صورة الطفل</p>
                        <p class="text-[11px] text-slate-400">اضغط على أيقونة الكاميرا لتغيير أو رفع صورة جديدة</p>
                    </div>
                </div>

                <div class="flex items-center gap-4">
                    <div class="text-right">
                        <span class="text-[11px] font-extrabold text-slate-400 block">كود الطفل:</span>
                        <input type="text" name="code" value="{{ old('code', $child->code) }}" required class="font-mono px-3 py-1 bg-white border border-slate-200 rounded-xl text-sm font-black text-slate-800 shadow-xs inline-block mt-0.5 w-28 text-center outline-none">
                    </div>
                    ...
                    <div class="text-right">
                        ...
                        <select name="status" class="px-3 py-1.5 bg-white border border-slate-200 rounded-xl text-xs font-bold outline-none mt-0.5 shadow-xs">
                            <option value="active" {{ old('status', $child->status) === 'active' ? 'selected' : '' }}> نشط في الخطة التأهيلية</option>
                            <option value="on_hold" {{ old('status', $child->status) === 'on_hold' ? 'selected' : '' }}> معلق / انتظار</option>
                            <option value="discharged" {{ old('status', $child->status) === 'discharged' ? 'selected' : '' }}> منتهي / متخرج</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-4 gap-5 text-xs font-medium">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="name" required value="{{ old('name', $child->name) }}" placeholder="مثال: سيف الدين محمود السعدني" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:bg-white font-bold text-sm">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="birth_date" required value="{{ old('birth_date', $child->birth_date ? $child->birth_date->format('Y-m-d') : '') }}" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:bg-white font-bold">
                    </div>

                    <div>
                        <label class="block font-bold text-slate-700 mb-1.5">العمر العقلي / اللغوي (إن وجد)</label>
                        <input type="text" name="mental_age" value="{{ old('mental_age', $child->mental_age) }}" placeholder="مثال: 3 سنوات ونصف أو 40 شهر" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:bg-white font-bold text-slate-800">
                    </div>

                    <div class="sm:col-span-3">
                        <label class="block font-bold text-slate-700 mb-1.5">عنوان السكن والمنطقة</label>
                        <input type="text" name="address" value="{{ old('address', $child->address) }}" placeholder="مثال: مدينة نصر - الحي السابع - شارع الطيران" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:bg-white font-medium">
                    </div>
            ...
            <div class="space-y-4 pt-4 border-t border-slate-100">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-4 gap-5 text-xs font-medium">
                    <div>
                        ...
                        <input type="text" name="national_id" required value="{{ old('national_id', $child->national_id) }}" maxlength="14" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:bg-white font-bold">
                    </div>

                    <div>
                        <label class="block font-bold text-slate-700 mb-1.5">اسم ولي الأمر <span class="text-rose-500">*</span></label>
                        <input type="text" name="parent_name" required value="{{ old('parent_name', $child->parent_name) }}" placeholder="مثال: أ. محمود السعدني" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:bg-white font-bold">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="parent_relation" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:bg-white font-bold">
                            <option value="الأب" {{ old('parent_relation', $child->parent_relation) === 'الأب' ? 'selected' : '' }}>الأب</option>
                            <option value="الأم" {{ old('parent_relation', $child->parent_relation) === 'الأم' ? 'selected' : '' }}>الأم</option>
                            <option value="الوصي / ولي الأمر" {{ old('parent_relation', $child->parent_relation) === 'الوصي / ولي الأمر' ? 'selected' : '' }}>الوصي / ولي الأمر</option>
                        </select>
                    </div>

                    <div>
                        <label class="block font-bold text-slate-700 mb-1.5">رقم هاتف ولي الأمر (الواتساب) <span class="text-rose-500">*</span></label>
                        <input type="text" name="phone" required value="{{ old('phone', $child->phone) }}" placeholder="01012345678" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:bg-white font-mono font-bold">
                    </div>

                    <div class="sm:col-span-3">
                        <label class="block font-bold text-slate-700 mb-1.5">رقم هاتف إضافي (للطوارئ)</label>
                        <input type="text" name="emergency_phone" value="{{ old('emergency_phone', $child->emergency_phone) }}" placeholder="رقم هاتف آخر أو هاتف المنزل" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:bg-white font-mono">
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 text-xs font-medium">
                    ...
                    <div>
                        ...
                        <select name="main_specialist" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:bg-white font-bold">
                            ...
                            <option value="{{ $spec }}" {{ old('main_specialist', $child->main_specialist) === $spec ? 'selected' : '' }}>{{ $spec }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="assistive_devices" value="{{ old('assistive_devices', $child->assistive_devices) }}" placeholder="مثال: قوقعة إلكترونية، سماعة طبية، نظارة" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:bg-white">
                    </div>

                    <div>
                        <label class="block font-bold text-slate-700 mb-1.5">التاريخ المرضي وملاحظات الحمل والولادة</label>
                        <input type="text" name="medical_notes" value="{{ old('medical_notes', $child->medical_notes) }}" placeholder="نقص أكسجين عند الولادة، عمليات جراحية، إلخ" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:bg-white">
                    </div>
