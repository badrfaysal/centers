...
with open('resources/views/doctor/index.blade.php', 'r', encoding='utf-8') as f:
    c = f.read()

start_str = '        <div class=\"space-y-4\">\n            @forelse(->groupBy(\'child_id\')'
end_str = '            @endforelse\n        </div>\n    </div>'

start_idx = c.find(start_str)
end_idx = c.find(end_str, start_idx) + len(end_str)

replacement = '''        <!-- الرسائل مجمعة حسب الطفل -->
        <div class="space-y-4">
            @forelse( as  => )
...
if start_idx != -1:
    ...
    with open('resources/views/doctor/index.blade.php', 'w', encoding='utf-8') as f:
        f.write(new_c)
    print("Done")
else:
